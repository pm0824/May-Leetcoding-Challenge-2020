...
# Solution: Using Depth first search algorithm

class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, newColor: int) -> List[List[int]]:
        connections = [(0, -1), (0, 1), (-1, 0), (1, 0)]

        def dfs(image, r, c, newColor, prevcolor):
            if (0 > r or r >= len(image) or 0 > c or c >= len(image[0]) or image[r][c] != prevcolor):
                return

            image[r][c] = newColor
            for d in connections:
                dfs(image, r+d[0], c+d[1], newColor, prevcolor)
